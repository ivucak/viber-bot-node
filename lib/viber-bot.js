...
function ViberBot(loggerOrConfiguration, configuration) {
	...
	this._middleware = new Middleware(this._logger, new MessageValidator(this._logger, this.authToken), configuration.app, configuration.path);
...
ViberBot.prototype.removeWebhook = function(publicKey, name, icon, app) {
	return new Promise(function(resolve, reject){
		try{
			let newBot = new ViberBot(null, {authToken: publicKey, path: "", name: name, avatar: icon, app: app});
			return newBot.setWebhook("");
		}catch(err){
			reject(err);
		}
	});
