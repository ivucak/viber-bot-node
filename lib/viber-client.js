...
ViberClient.prototype.removeWebhook = function(url, isInline) {
	this._logger.info(`Removing webhook. Sending 'setWebhook' request for url: ${url}, isInline: ${isInline}`);
	return this._sendRequest("setWebhook", {
		"url": url,
		"is_inline": isInline,
		"event_types": this._subscribedEvents
...
ViberClient.prototype._sendRequest = function(endpoint, data) {
	...
	this._logger.debug(`Opening request to url: '${url}' with ${JSON.stringify(data)}`);
